...
$.fn.progressBar = function(todo)
{
	...
	var self = {
		...
		options:
		{
			...
			step: 1, // grid step
			initial: false, // initial value, integer
			eventExtArea: false,
		...
		return this.each(function(){
			...
			if(data.opts.interactive)
			{
				var muCb = function()
				{
					...
					{
						if(!data.vars.notClick) self._processMouse.call(data); // simple click (i.e. there was no mousemove between mousedown and mouseup)
	...
	self._setValue = function(progress, strict){
		...
		progress = +progress;
		if(strict && (progress < 0 || progress > top)) return; // exact interval match
		if(progress < 0) progress += top; // negative values bring back to the interval
		if(progress > top) progress %= top; // too high values also
